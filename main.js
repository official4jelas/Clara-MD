...
const _dirname = global.__dirname(import.meta.url)
...
function clearSessions(folder = 'sessions') {
    ...
    return filename.map(file => {
        ...
        if (stats.isFile() && (Date.now() - stats.mtimeMs >= 1000 * 60 * 120)) { // 2 hours
...
async function connectionUpdate(update) {
    ...
    if (isOnline) console.log(chalk.green('Status Aktif'))
    if (!isOnline) console.log(chalk.red('Status Mati'))
...
const pluginFolder = global.__dirname(join(_dirname, './plugins/index'))
...
await filesInit().then(_ => console.log(Object.keys(global.plugins))).catch(console.error)
